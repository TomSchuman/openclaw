...
describe("printDaemonStatus", () => {
  ...
  it("reports an admitted probe timeout as event-loop saturation instead of failed connectivity", () => {
    ...
    expectMockLineContains(runtime.error, "Read probe: timed out under event-loop load");
    expectMockLineContains(
      runtime.error,
      "Gateway event loop: degraded max=5100ms p99=5079ms util=1 cpu=0.94",
    );
    expectMockLineContains(runtime.log, "Gateway accepted the connection");
    expect(runtime.error.mock.calls.flat().join("\n")).not.toContain("Connectivity probe: failed");
    expect(runtime.log.mock.calls.flat().join("\n")).not.toContain("not a warm-up delay");
  });
